...
+ (NSMutableArray *)loadData { //+ (Chapters *)loadData {
    ...
    for (GDataXMLElement *element in dataArray) {
        ...
        NSArray *textArray = [element elementsForName:@"Text"];
        ...
        if (nameArray.count>0)
        {
            GDataXMLElement *textElement = (GDataXMLElement *) [textArray objectAtIndex:0];
            text = [textElement stringValue];
        }
   
        Chapter *chapter = [[Chapter alloc] initWithName:name number:number text:text];
