...
- (CCLayer*)layerWithChapterName:(NSString*)chapterName 
    ...
    if (self.iPad) {
        ...
        CCMenuItemImage *image = [CCMenuItemImage itemWithNormalImage:@"StickyNote-iPhone.png" 
    ...
    CCLabelTTF *layerLabel = [CCLabelTTF labelWithString:chapterText fontName:@"Marker Felt" fontSize:largeFont];
...
- (id)init {
    ...
    if( (self=[super init])) {
        ...
        for (Chapter *chapter in chapters) { //for (Chapter *chapter in chapters.chapters) {
            ...
            CCLayer *layer = [self layerWithChapterName:chapter.name chapterNumber:chapter.number chapterText:chapter.text screenSize:screenSize];
        ...
        [scroller release];
        [layers release];
        DeviceManager *device111 = [[DeviceManager alloc] init];
        [self addChild:device111];
        [self addBackButton];  
